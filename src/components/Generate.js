...
const Generate = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
        try {
            const { data: approvedLoansRes } = await axios.get('http://localhost:3001/loans/status/count');
            ...
            const { data: totalLoanRes } = await axios.get('http://localhost:3001/loan/total-loan-amount');
            const { data: repayRes } = await axios.get('http://localhost:3001/loan/payment-status');
